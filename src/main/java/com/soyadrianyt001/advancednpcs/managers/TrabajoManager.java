...
public class TrabajoManager {
    ...
    private void moverFancyNPC(NPCEntity npc, Location loc) {
        try {
            String fancyId = "advancednpc_" + npc.getId();
            Npc fancyNpc = FancyNpcsPlugin.get().getNpcManager().getNpc(fancyId);
            if (fancyNpc != null) {
                fancyNpc.getData().setLocation(loc);
            ...
            .runTaskTimer(plugin, () -> {
                ...
                for (int x = -3; x <= 3 && !aguaCerca; x++)
                    for (int z = -3; z <= 3 && !aguaCerca; z++)
                        if (loc.clone().add(x, -1, z).getBlock()
        ...
        BukkitTask task = plugin.getServer().getScheduler()
            .runTaskTimer(plugin, () -> {
                ...
                for (org.bukkit.entity.Animals a :
                    loc.getWorld().getEntitiesByClass(
